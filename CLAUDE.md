...
- **Seven sections**, closed and positional: `Data`, `Training`, `Post-Training`, `Agents`, `Governance`, `Economics`, `Misc`. The canonical `Governance` section is displayed to readers as **Strategy & Governance**; `Economics` is displayed as **Economics & Social**. A note's `section` front-matter field must match its parent directory exactly.
