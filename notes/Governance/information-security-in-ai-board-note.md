---
title: "Information Security That Enables Safe Speed"
description: "A board guide to information security when AI accelerates both work and attack: reserved decisions, safe sharing paths, exfiltration routes, and bad-day readiness."
kind: "guide"
section: "Governance"
...
# Information Security That Enables Safe Speed
...
**Thesis:** Security should enable safe speed, not maximum restriction. Trust people to work and report mistakes. Do not grant systems implicit trust. Verify identity, high-consequence actions, system behavior, and AI claims in proportion to harm. Friction should rise with sensitivity and consequence, not for every document, email, call, or AI query.

We trust the team to do their jobs. We still verify systems and detect problems as early as possible. Trust belongs to people. Verification and early detection apply to systems, data flows, actions, and emerging anomalies, not to watching staff for its own sake.

AI does not chiefly create a wholly new category of risk. It compresses the cost and time needed to run familiar attack steps at scale: reconnaissance, tailored phishing and impersonation, vulnerability research, basic malware generation, and finding value in stolen data. The NCSC assesses that AI will increase the frequency and intensity of cyber threats and spread intrusion capability more widely. Fully automated end-to-end advanced attacks remain unlikely through 2027. Capable people remain in the loop.[10]
...
## 4. The five sharing paths

Information does not move only through email. The board should require practical rules for the five routes staff and systems use every day.[9]
...
The sharing paths describe how work should flow. The board should also see the routes by which data actually leaves organizations, whether by mistake, by a compromised account, or by intent. This list draws on the MITRE ATT&CK exfiltration catalogue and NCSC insider guidance.[9][15]
...
## 7. Apply zero trust to systems, not as a judgment on people
...
Verification should follow consequence. We trust the team to do their jobs. Verification and early detection are directed at systems, data flows, actions, and emerging anomalies, not routine surveillance of staff. High-consequence AI outputs and actions need human review or another independent control. Test after a material change to a model, data source, prompt, connector, or deployment path. Protect logs, which may contain sensitive content and the record of how an action occurred. Rehearse revocation and incident response before an urgent case makes improvisation attractive.[6]

One counterpoint matters. DMARC, TLS, and data-loss-prevention tools are useful controls, but none proves a message is safe. An enterprise AI label does not prove appropriate data use. A human click does not prove that a decision was sound. Controls reduce risk. They do not erase judgment.
...
4. **Dynamic detection and response:** Can management detect changing risk across sharing paths, get a human to a high-risk case quickly, contain it, learn, and update controls? The evidence should include time from anomaly to human decision, time to contain or revoke, false-positive and bypass rates, and lessons from incidents and exercises.
...
- **Trust and verify:** Trust staff to work and report mistakes. Check systems, data flows, and high-consequence actions early enough to stop or contain harm. This is not routine surveillance of staff.[6]

- **Phishing-resistant MFA:** Sign-in that cannot be captured and replayed by a fake site or a persuasive caller, in practice passkeys or hardware security keys. Codes sent by text or generated by an app can be phished; these cannot.[14]

- **Prompt injection:** Instructions hidden in content an AI system reads (a web page, document, or email) that hijack the system into leaking data or misusing its permissions. The top-ranked risk in the OWASP list for AI applications, and the core reason prompts are not an access-control boundary.[18]
...
- **STRIDE:** A design-review method for product and security teams, not a board operating principle. It asks whether someone could impersonate a user or service, alter data, deny an action, disclose information, disrupt a service, or gain excess privilege. Use it when approving a material new AI system, connector, or data flow.[12]
...
9. [NCSC guidance on reducing data exfiltration by malicious insiders](https://www.ncsc.gov.uk/guidance/reducing-data-exfiltration-by-malicious-insiders) — UK government guidance; supports the sharing-path rules and exfiltration routes.
