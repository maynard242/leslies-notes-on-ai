...
> **Concepts to know:** *Shadow AI* — unsanctioned tools staff adopt outside procurement, invisible to any inventory that only counts approved systems. *Embedded AI* — AI features shipped inside vendor or SaaS products the company didn't build and may not know are there. *Agentic AI* — systems that take actions — spend, send, sign — rather than only produce text or a recommendation. *Foundation model* — the large pretrained model (GPT, Claude, Gemini, Llama, and similar) that most vendor features and in-house builds actually sit on top of; two unrelated-looking products can share the same underlying model and its failure modes. *Fine-tuning, RAG, and prompting* — the different ways a foundation model gets adapted to a company's use — retrained on company data, given company documents to consult at answer time, or simply instructed — each with a different data-exposure profile worth its own inventory line. *Multimodal AI* — models that process or generate images, audio, and video as well as text; inventories that only look for "chatbots" routinely miss voice systems, image screening tools, and video moderation running elsewhere in the business. *Automated decision-making (ADM)* — the term regulators use for AI output that makes or heavily influences a decision about a person with no meaningful human involvement; the direct hook behind which uses touch hiring, credit, or insurance.
...
> **Concepts to know:** *Model risk management (MRM)* — lifecycle ownership, validation, and monitoring assigned to a model independent of who built it, a discipline borrowed from banking regulation and the substance behind "ownership" in this section. *Model versioning* — vendors update the model behind a product continuously; the system a risk owner is accountable for today may not be the same model in three months, and ownership has to track which version is actually in production. *Training versus inference* — building a model (often one-time, vendor-side) versus using it (continuous, and mostly under the company's own control); most of a company's controllable risk sits in the second, even when the model itself was built by someone else. *Foundation-model dependency* — when several business units' "different" systems are really thin layers over the same underlying model, ownership at the boundary has to account for one shared failure mode, not several separate ones. *Model lifecycle* — the stages a model moves through: data collection, training, evaluation, deployment, monitoring, retirement; ownership assigned only at "deployment" misses risk accumulating at every other stage. *Vendor (or fourth-party) risk* — most companies do not train their own foundation model, so a large share of "model" risk actually sits one layer further out, with the vendor's own upstream model supplier, outside anyone in the room's direct control.
...
> **Concepts to know:** *Human-in-the-loop / human-on-the-loop / human-out-of-the-loop* — the spectrum from a person approving every action, to a person who can intervene but need not, to no person in the decision at all; the technical version of "assists a decision" versus "replaces one." *Autonomy ceiling* — the explicit limit on what an agent may do unsupervised: spend, contract, speak for the company. *Guardrails* — the technical filters, classifiers, and rules wrapped around a model that actually enforce a "will not do" boundary or an autonomy ceiling, as distinct from the policy that states it. *Temperature (or sampling)* — the setting controlling how deterministic versus varied a model's output is for the same input; one lever, not the only one, for keeping a system inside an approved failure-rate tolerance. *Context window* — how much information a model can weigh at once; a decision handed to a system with too little context is a different risk than the same decision handed to a person who has all of it. *Confidence score (or calibration)* — a model's own estimate of how likely its output is correct; a well-calibrated system flags its uncertain answers for human review, a poorly calibrated one sounds equally sure whether right or wrong. *Function calling (or tool use)* — the mechanism that lets a model actually invoke another system — send an email, place an order, query a database — rather than only produce text; the literal capability behind an agent's ability to spend, contract, or speak for the company at all.
...
> **Concepts to know:** *Red-teaming* — deliberately trying to break a system before deployment, distinct from ordinary testing that only checks intended behavior. *Evals* — structured tests scored against defined pass criteria; what "what did testing check, against what pass criteria" is really asking to see. *Model drift* — a deployed system's behavior quietly changing as the world, the inputs, or an updated backend model shift underneath it, with nothing retrained or redeployed on the company's side. *Hallucination (or confabulation)* — fluent, confident output that is factually wrong or invented; the archetypal malfunction a monitoring regime has to catch. *Prompt injection* — malicious instructions hidden inside a document, email, or webpage the model processes, hijacking its behavior without the user typing anything harmful. *Jailbreak* — techniques that get a model to bypass its own safety training and produce what it was built to refuse; the other standard red-team target alongside prompt injection. *Observability* — the logging, tracing, and telemetry that let a team see what a production system actually decided and why, not just whether it stayed up; the infrastructure "what runs in production monitoring" assumes exists. *Out-of-distribution input* — a real-world input that differs from anything the model was trained or tested on; the standard reason a system that looked reliable in testing fails unpredictably once it meets the world.
...
> **Concepts to know:** *Model card (or system card)* — a standardized document describing a model's intended use, training data, evaluation results, and known limits; what a vendor should be able to hand over as test results, not an assurance letter. *Training data provenance* — where a model's training data came from and under what license or permission; usually opaque for a vendor model, and a specific evidentiary gap between an in-house and a bought system. *System (or inference) logs* — the actual record of inputs, outputs, and decisions a deployed system produced, as distinct from a policy document describing what it is supposed to do; the literal trail behind "can it trace who approved a given deployment." *Benchmark-to-production gap* — a vendor's cited number ("95% accuracy on X benchmark") describes performance on a fixed test set, not a guarantee about the company's actual data, users, and edge cases. *Open weights versus API access* — whether the company holds the actual model file, and can run its own tests and roll it back, or only calls a vendor's hosted endpoint and depends entirely on the vendor's own records; a first-order fact for what a company can prove about a system it does not run itself. *Reproducibility* — whether the same test on the same inputs produces a comparable result later; harder than it sounds when a vendor can update the model, its filters, or its infrastructure with no notice.
